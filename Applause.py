...
class Applause:
    ...
    def display_plate(self, param_cmap, param_aspect, param_origin, raoffset, decoffset):
        ...
        plt.xlabel('RA')
        plt.ylabel('DEC')
        plt.title(str(self.object) + ' ' + str(self.scan_id_uq[0]) + ' ' + str(self.ut_mid_matrix[0]) + ' ' + str(
    ...
    def display_observation(self, fov, param_cmap, param_aspect, param_origin, alpha):
        ...
        yexminz = dec - ysz
        yexmaxz = dec + ysz
        plt.imshow(self.dataobs, cmap=param_cmap, extent=(xexminz, xexmaxz, yexminz, yexmaxz), aspect=param_aspect,
                   origin=param_origin, alpha=alpha)
        plt.xlabel('RA')
        plt.ylabel('DEC')
        plt.title("DSS1")
    ...
    def display_plate_zoom(self, fov, param_cmap, param_aspect, param_origin, alpha, dsp, raoffset, decoffset):
        ...
        if dsp == 1:
            plt.imshow(self.plate[iyminz + idecoffset:iymaxz + idecoffset, ixminz + iraoffset:ixmaxz + iraoffset],
                       cmap=param_cmap, extent=(xexminz, xexmaxz, yexminz, yexmaxz), aspect=param_aspect,
                       origin=param_origin, alpha=alpha)
        else:
            print('RAmax: ', self.calc_ra(xexminz), 'RAmin: ', self.calc_ra(xexmaxz), 'DECmax: ',
                  self.calc_dec(yexminz), 'DECmin: ', self.calc_dec(yexmaxz))

        plt.xlabel('RA')
        plt.ylabel('DEC')
        plt.rcParams.update({'axes.titlesize': 'small'})
        plt.title(str(self.object) + ' ' + str(self.plate_id_uq[0]) + ' ' + str(self.ut_mid_matrix[0]) + ' ' + str(
            np.around(self.objmag, decimals=3)))
    ...
    def get_bndry(self, param):
        ...
        bndry = {'minra': np.min(self.raj2000_matrix), 'maxra': np.max(self.raj2000_matrix),
                 'minde': np.min(self.dej2000_matrix), 'maxde': np.max(self.dej2000_matrix)}
        return bndry[param]
